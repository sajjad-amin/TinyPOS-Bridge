"""
Hardware specifications and Bainiu thermal printer protocol constants.
"""

from typing import Dict, List

# Standard 57mm thermal printer width in dots (384 pixels = 48 bytes per row)
PRINT_WIDTH: int = 384

# Bainiu / Tiny Print BLE Service & Characteristic UUIDs
KNOWN_SERVICE_UUIDS: List[str] = [
    "0000ae30-0000-1000-8000-00805f9b34fb",
    "0000af30-0000-1000-8000-00805f9b34fb",
    "0000ff00-0000-1000-8000-00805f9b34fb",
    "49535343-fe7d-4ae5-8fa9-9fafd205e455",
]

KNOWN_TX_CHAR_UUIDS: List[str] = [
    "0000ae01-0000-1000-8000-00805f9b34fb",
    "0000ff02-0000-1000-8000-00805f9b34fb",
    "49535343-8841-43f4-a8d4-ecbe34729bb3",
]

# Advertised BLE device names for Bainiu / Tiny Print family
KNOWN_NAMES: List[str] = [
    "x6", "tiny print", "tinyprint", "iprint", "pocket printer",
    "gb01", "x5", "x7", "x2", "x1", "x8", "sc03", "rt034", "mx0", "c9", "c15",
]

# Bainiu Protocol Framing
HEADER: bytes = b"\x51\x78"
FOOTER: bytes = b"\xff"

# Standard Bainiu CRC8 Lookup Table
CRC8_TABLE: List[int] = [
    ,
]

# Exact command sequences matching Tiny Print / X5 / X6 firmware
CMD_GET_DEV_STATE: bytes = bytes.fromhex("5178A30001000000FF")
CMD_SET_QUALITY_200_DPI: bytes = bytes.fromhex("5178A4000100329EFF")
CMD_APPLY_ENERGY: bytes = bytes.fromhex("5178BE0001000107FF")
CMD_LATTICE_START: bytes = bytes.fromhex("5178A6000B00AA551738445F5F5F44382CA1FF")
CMD_LATTICE_END: bytes = bytes.fromhex("5178A6000B00AA5517000000000000001711FF")
CMD_FEED_PAPER: bytes = bytes.fromhex("5178A10002006000F5FF")

# Maps Unicode script categories to bundled font files
SCRIPT_FONT_FILE: Dict[str, str] = {
    "bengali": "SolaimanLipi.ttf",
    "arabic": "NotoSansArabic-Bold.ttf",
    "devanagari": "NotoSansDevanagari-Bold.ttf",
    "tamil": "NotoSansTamil-Bold.ttf",
    "telugu": "NotoSansTelugu-Bold.ttf",
    "thai": "NotoSansThai-Bold.ttf",
    "korean": "NotoSansKR-Bold.ttf",
    "hebrew": "NotoSansHebrew-Bold.ttf",
    "gujarati": "NotoSansGujarati-Bold.ttf",
    "gurmukhi": "NotoSansGurmukhi-Bold.ttf",
    "kannada": "NotoSansKannada-Bold.ttf",
    "malayalam": "NotoSansMalayalam-Bold.ttf",
    "oriya": "NotoSansOriya-Bold.ttf",
    "sinhala": "NotoSansSinhala-Bold.ttf",
    "myanmar": "NotoSansMyanmar-Bold.ttf",
    "khmer": "NotoSansKhmer-Bold.ttf",
    "lao": "NotoSansLao-Bold.ttf",
    "georgian": "NotoSansGeorgian-Bold.ttf",
    "armenian": "NotoSansArmenian-Bold.ttf",
    "ethiopic": "NotoSansEthiopic-Bold.ttf",
    "cjk": "NotoSansSC-Bold.ttf",
    "cyrillic": "NotoSans-Bold.ttf",
    "emoji": "NotoEmoji-Regular.ttf",
    "latin": "NotoSans-Bold.ttf",
}

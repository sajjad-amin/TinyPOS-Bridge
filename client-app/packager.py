...
def ensure_built(target_os: str, version: str = DEFAULT_VERSION, keep_cache: bool = False):
    ...
    if target_os == "Darwin":
        ...
        app_target = DIST_DIR / f"{APP_NAME}.exe"
    else:
        app_target = DIST_DIR / APP_NAME

    if not app_target or not app_target.exists():
        print_step(f"Pre-compiled binary '{app_target.name if app_target else target_os}' not found. Invoking build.py...")
        build_script = CLIENT_DIR / "build.py"
        cmd = [sys.executable, str(build_script), "--version", version]
        if keep_cache:
    ...
    if app_target and app_target.exists():
        print_success(f"Verified standalone binary: {app_target.name}")
    else:
